...
//        List<FDCandidate> fdCandidates = new ArrayList<>();
        ...
        FDTreeArray.FDTreeNode node = result.getRoot();
...
//        Map<Integer, Boolean[]> attributeToConfirmed = new HashMap<>();
        ...
        for(int i = 0; i < attributeNum; i++){
            FDTreeArray.FDTreeNode child = result.new FDTreeNode(node, i, attributeNum,null);
...
//                    fdCandidates.add(new FDCandidate(left, k));
                }
            }
            attributeToConfirmed.put(i,child.RHSCandidate);
            node.children.add(child);
            queue.add(new FDDiscoverNodeSavingInfoArray(child, fdTreeNodeEquivalenceClasses, left));
        }
//        for(int i = 0; i < attributeToConfirmed.size(); i++){
//            System.out.println("LHS:"+ i);
//            System.out.println("comfirmed:");
//            printArray(attributeToConfirmed.get(i));
//        }
//        System.out.println();
        while (!queue.isEmpty()){
...
//                    prune规则4     XY->Z,则XYZ->M否由XY->M否决定，XY->M成立则不是最小，XY->M不成立则不成立
                    ...
                    FDTreeArray.FDTreeNode child = result.new FDTreeNode(parent, i, attributeNum, attributeToConfirmed.get(i));
...
//                    System.out.println("LHS:" + left);
                    for(int k = 0; k < attributeNum; k++){
//                        System.out.println("RHS:" + k);
//                        若已经在父结点成立，则必成立，不用验证
                        if(child.RHSCandidate[k]){
//                            System.out.println("parent中已成立");
...
//                                System.out.println("valid");
                    ...
                    child.minimal = checkFDMinimalArray(child, parent, left, fdCandidates, attributeToConfirmed.get(i));
...
//                            fdCandidates.add(new FDCandidate(left, j));
                    ...
                    queue.offer(new FDDiscoverNodeSavingInfoArray(child, fdTreeNodeEquivalenceClasses, left));
                }
            }
        }
